...
from scripts.main.APL_New import NFL_RCE
...
model_name = "NFL_RCE_noisy_labels_0.6"
criterion = NFL_RCE()
...
scheduler = torch.optim.lr_scheduler.OneCycleLR(
    optimizer, max_lr=0.001, steps_per_epoch=len(train_loader), epochs=total_epochs
)
...
plt.figure()
plt.plot(range(1, total_epochs +1), losses, label='Loss', color='red')
plt.xlabel('Epochs')
plt.ylabel('Loss')
...
plt.figure()
plt.plot(range(1, total_epochs + 1), accuracies, label='Accuracy', color='blue')
plt.xlabel('Epochs')
plt.ylabel('Accuracy (%)')
